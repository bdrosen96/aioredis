...
class ServerCommandsMixin:
    ...
    def bgrewriteaof(self):
        ...
        fut = self._execute(b'BGREWRITEAOF')
        return wait_ok(fut)

    def bgsave(self):
        """Asynchronously save the dataset to disk."""
        fut = self._execute(b'BGSAVE')
        return wait_ok(fut)
    ...
    def client_list(self):
        ...
        fut = self._execute(b'CLIENT', b'LIST', encoding='utf-8')
        return wait_convert(fut, to_tuples)

    def client_getname(self, encoding=_NOTSET):
        """Get the current connection name."""
        return self._execute(b'CLIENT', b'GETNAME', encoding=encoding)
    ...
    def client_pause(self, timeout):
        ...
        fut = self._execute(b'CLIENT', b'PAUSE', timeout)
        return wait_ok(fut)

    def client_setname(self, name):
        """Set the current connection name."""
        fut = self._execute(b'CLIENT', b'SETNAME', name)
        return wait_ok(fut)
    ...
    def config_get(self, parameter):
        ...
        fut = self._execute(b'CONFIG', b'GET', parameter)
        return wait_convert(fut, to_dict)

    def config_rewrite(self):
        """Rewrite the configuration file with the in memory configuration."""
        fut = self._execute(b'CONFIG', b'REWRITE')
        return wait_ok(fut)
    ...
    def config_set(self, parameter, value):
        ...
        if not isinstance(parameter, str):
            raise TypeError("parameter must be str")
        fut = self._execute(b'CONFIG', b'SET', parameter, value)
        return wait_ok(fut)

    def config_resetstat(self):
        """Reset the stats returned by INFO."""
        fut = self._execute(b'CONFIG', b'RESETSTAT')
        return wait_ok(fut)

    def dbsize(self):
        """Return the number of keys in the selected database."""
        return self._execute(b'DBSIZE')

    def debug_object(self, key):
        """Get debugging information about a key."""
        return self._execute(b'DEBUG', b'OBJECT', key)

    def debug_segfault(self, key):
        """Make the server crash."""
        return self._execute(b'DEBUG', 'SEGFAULT')

    def flushall(self):
        """Remove all keys from all databases."""
        fut = self._execute(b'FLUSHALL')
        return wait_ok(fut)

    def flushdb(self):
        """Remove all keys from the current database."""
        fut = self._execute('FLUSHDB')
        return wait_ok(fut)
    ...
    def lastsave(self):
        ...
        return self._execute(b'LASTSAVE')
    ...
    def role(self):
        ...
        return self._execute(b'ROLE')

    def save(self):
        """Synchronously save the dataset to disk."""
        return self._execute(b'SAVE')
    ...
    def shutdown(self, save=None):
        ...
        if save is self.SHUTDOWN_SAVE:
            return self._execute(b'SHUTDOWN', b'SAVE')
        elif save is self.SHUTDOWN_NOSAVE:
            return self._execute(b'SHUTDOWN', b'NOSAVE')
        else:
            return self._execute(b'SHUTDOWN')
    ...
    def slaveof(self, host=None, port=None):
        ...
        if host is None and port is None:
            return self._execute(b'SLAVEOF', b'NO', b'ONE')
        return self._execute(b'SLAVEOF', host, port)
    ...
    def slowlog_get(self, length=None):
        ...
        if length is not None:
            ...
            return self._execute(b'SLOWLOG', b'GET', length)
        else:
            return self._execute(b'SLOWLOG', b'GET')

    def slowlog_len(self, length=None):
        """Returns length of Redis slow queries log."""
        return self._execute(b'SLOWLOG', b'LEN')

    def slowlog_reset(self):
        """Resets Redis slow queries log."""
        return self._execute(b'SLOWLOG', b'RESET')

    def sync(self):
        """Redis-server internal command used for replication."""
        return self._execute(b'SYNC')

    def time(self):
        """Return current server time."""
        fut = self._execute(b'TIME')
