...
class ScriptingCommandsMixin:
    ...
    def eval(self, script, keys=[], args=[]):
        ...
        return self._execute(b'EVAL', script, len(keys), *(keys + args))
    ...
    def script_exists(self, digest, *digests):
        ...
        return self._execute(b'SCRIPT', b'EXISTS', digest, *digests)

    def script_kill(self):
        """Kill the script currently in execution."""
        fut = self._execute(b'SCRIPT', b'KILL')
        return wait_ok(fut)

    def script_flush(self):
        """Remove all the scripts from the script cache."""
        fut = self._execute(b"SCRIPT",  b"FLUSH")
        return wait_ok(fut)
